...
@interface TMPhotoBrowser () <TMZoomingScrollViewDelegate, UIScrollViewDelegate>
...
- (void)didInitialize {
    self.backgroundColor = TMPhotoBrowserBackgroundColor;
    self.visibleZoomingScrollViews = [[NSMutableSet alloc] init];
    self.reusableZoomingScrollViews = [[NSMutableSet alloc] init];
    [self placeholderImage];
    
    _pageControlAliment = TMPhotoBrowserPageControlAlimentCenter;
    _pageControlStyle = TMPhotoBrowserPageControlStyleAnimated;
    _pageControlDotSize = CGSizeMake(10, 10);
    _hidesForSinglePage = YES;
    _currentPageDotColor = [UIColor whiteColor];
    _pageDotColor = [UIColor lightGrayColor];
    _browserStyle = TMPhotoBrowserStylePageControl;
...
- (void)orientationDidChange {
    ...
    TMZoomingScrollView *temp = [self zoomingScrollViewForIndex:self.currentIndex];
...
- (void)updateFrames {
    ...
    rect.size.width += TMPhotoBrowserImageViewMargin;
    ...
    if ([self.pageControl isKindOfClass:[TMPageControl class]]) {
        TMPageControl *pageControl = (TMPageControl *)self.pageControl;
    ...
    switch (self.pageControlAliment) {
        case TMPhotoBrowserPageControlAlimentLeft:
        {
            x = 20;
        }
            break;
        case TMPhotoBrowserPageControlAlimentCenter:
        {
            x = (self.lt_width - size.width) * 0.5;
        }
            break;
        case TMPhotoBrowserPageControlAlimentRight:
        {
...
- (void)showPhotos {
    ...
    for (TMZoomingScrollView *zoomingScrollView in self.visibleZoomingScrollViews) {
...
- (BOOL)isShowingZoomingScrollViewAtIndex:(NSInteger)index {
    for (TMZoomingScrollView *zoomScrollView in self.visibleZoomingScrollViews) {
...
/**
 *  获取指定位置的TMZoomingScrollView对象，显示池，回收池，创建新对象
 */
- (TMZoomingScrollView *)zoomingScrollViewForIndex:(NSInteger)index {
    for (TMZoomingScrollView *zoomingScrollView in self.visibleZoomingScrollViews) {
        if (zoomingScrollView.tag - BaseTag == index) {
            return zoomingScrollView;
        }
    }
    TMZoomingScrollView *zoomingScrollView = [self dequeueReusableZoomingScrollView];
    [self setupImageForZoomingScrollViewAtIndex:index];
    return zoomingScrollView;
}
/**
...
/**
 *  从缓存池中获取一个TMZoomingScrollView对象
 */
- (TMZoomingScrollView *)dequeueReusableZoomingScrollView {
    TMZoomingScrollView *photoView = [self.reusableZoomingScrollViews anyObject];
    if (photoView) {
        [self.reusableZoomingScrollViews removeObject:photoView];
    } else {
        photoView = [[TMZoomingScrollView alloc] init];
    }
    return photoView;
}
/**
...
- (void)showFirstImage {
    ...
    // 动画修改图片的frame，居中同时放大
    [UIView animateWithDuration:TMPhotoBrowserShowImageAnimationDuration animations:^{
...
#pragma mark    -   TMZoomingScrollViewDelegate

- (void)zoomingScrollView:(TMZoomingScrollView *)zoomingScrollView imageLoadProgress:(CGFloat)progress {
    
}

- (void)zoomingScrollView:(TMZoomingScrollView *)zoomingScrollView singleTapDetected:(UITapGestureRecognizer *)singleTap {
    ...
    [UIView animateWithDuration:TMPhotoBrowserHideImageAnimationDuration animations:^{
...
/**
 ...
 @return 图片浏览器对象
 */
+ (instancetype)showPhotoBrowserWithCurrentImageIndex:(NSInteger)currentImageIndex imageCount:(NSInteger)imageCount datasource:(id<TMPhotoBrowserDatasource>)datasource {
...
- (UIWindow *)photoBrowserWindow {
    if (!_photoBrowserWindow) {
        ...
        tempVC.view.backgroundColor = TMPhotoBrowserBackgroundColor;
...
- (void)setPageControlStyle:(TMPhotoBrowserPageControlStyle)pageControlStyle {
...
- (void)setBrowserStyle:(TMPhotoBrowserStyle)browserStyle {
    _browserStyle = browserStyle;
    [self updateIndexVisible];
}

- (void)setPageControlAliment:(TMPhotoBrowserPageControlAliment)pageControlAliment {
...
- (void)setupPageControl {
    ...
    switch (self.pageControlStyle) {
        case TMPhotoBrowserPageControlStyleClassic:
        {
            ...
            break;
        case TMPhotoBrowserPageControlStyleAnimated:
        {
            TMPageControl *pageControl = [[TMPageControl alloc] init];
            pageControl.currentPage = self.currentIndex;
            pageControl.numberOfPages = self.imageCount;
...
- (void)updateIndexVisible {
    ...
    switch (self.browserStyle) {
        case TMPhotoBrowserStylePageControl:
        {
            ...
            break;
        case TMPhotoBrowserStyleIndexLabel:
        {
            ...
            break;
        case TMPhotoBrowserStyleSimple:
        {
            self.indexLabel.hidden = NO;
