...
- (void)initValue {
    self.dotViewClass = [TMAnimatedDotView class];
...
- (UIView *)generateDotView {
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[TMAnimatedDotView class]] && self.dotColor) {
            ((TMAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index {
    if (self.dotViewClass) {
        TMBaseDotView *dotView = (TMBaseDotView *)[self.dots objectAtIndex:index];
        if ([dotView respondsToSelector:@selector(changeActivityState:)]) {
            [dotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [TMBaseDotView class]);
        }
